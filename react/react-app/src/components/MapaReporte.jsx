...
const MapaleafReporte = (gps) => {
    ...
    const[index, setIndex] = useState(gps.id)
    // if(gps.id){
    //   console.log(gps.id)
    // } else {
    ...
    // Map creation:
    useEffect(() => {
      mapRef.current = L.map("map"+index, mapParams);
    }, []);
    ...
    useEffect(() => {
      ...
      mpp.onAdd = function(map) {
        ...
        div.innerHTML += '<i id="iMpp'+index+'" style="background: #fff; width:18px; height:18px; float: left; margin-right: 8px; opacity: 0.7"></i><span id="spanMpp'+index+'">'+showMpp+'</span><br>';
    ...
    useEffect(()=> {
      ...
      var polygone = L.polyline(gps['gps'], {color: 'black', fill:false});
      ...
      function a(data){
        var mpp = L.DomUtil.get('spanMpp'+index);
        var c = L.DomUtil.get('iMpp'+index);
  ...
  return (
    ...
    <div>
      <div id={"map"+index} style={mapStyles} />
    </div>
...
export default MapaleafReporte